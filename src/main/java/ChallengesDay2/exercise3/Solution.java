package ChallengesDay2.exercise3;
...
class Result {
    ...
    public static List<Integer> countingSort(List<Integer> arrInput) {
        // Write your code here
       /* int[] tmpArray = new int[100];

        Integer[] boxedArray = Arrays.stream(tmpArray) // IntStream
                .boxed()                // Stream<Integer>
                .toArray(Integer[]::new);
        List<Integer> tmpArrayList = new ArrayList<Integer>(Arrays.<Integer>asList(boxedArray));

        List<Integer> auxList = new ArrayList<>();

        external:
        for (Integer itemExternal: arrInput) {
            for (Integer itemInternal: arrInput) {
                if (!auxList.contains(itemExternal)) {
                    if (itemExternal.compareTo(itemInternal)==0) {
                        tmpArrayList.set(itemExternal, tmpArrayList.get(itemExternal) + 1);
                    }
                } else {
                    continue external;
                }
            }
            auxList.add(itemExternal);
        }

     return tmpArrayList;*/

        return IntStream.rangeClosed(0,99).
                map(v -> Collections.frequency(arrInput, v))
                .boxed()
